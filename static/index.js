...
function getPixels() {
    let rawPixels = context.getImageDate(0, 0, 200, 200).data
    let _pixels = []
    let pixels = []
    ...
    // going from 200 x 200 to 50 x 50 and keeping the main shape of the data
    for (i=0; i < _pixels.length; i += 800) {
        // J += 4 helps in skipping every 4th pixel
        for (j=0; j < 200; j += 4) {
